...
class AskingOtherApprovalScreen extends StatefulWidget {
  final Map<String, dynamic>? deliveryData;

  const AskingOtherApprovalScreen({super.key, this.deliveryData});

  @override
  State<AskingOtherApprovalScreen> createState() => _AskingOtherApprovalScreenState();
}

class _AskingOtherApprovalScreenState extends State<AskingOtherApprovalScreen> {
  ...
  Widget _buildCompanyRow() {
    return Row(
      children: [
        CircleAvatar(radius: 13, backgroundImage: AssetImage(widget.deliveryData?['serviceLogo'] ?? 'assets/images/amazon_log.png')),
        const SizedBox(width: 5),
        Text(widget.deliveryData?['serviceName'] ?? 'Company', style: const TextStyle(fontSize: 16, color: Colors.black54)),
      ],
    );
  }
  ...
  void _askForApproval() {
    context.read<GuardEntryBloc>().add(AddDeliveryEntry(
      ...
      serviceName: widget.deliveryData?['serviceName'],
      serviceLogo: widget.deliveryData?['serviceLogo'],
